...
cd ~
wget http://apache.rediris.es/ant/binaries/apache-ant-1.10.3-bin.tar.gz
tar -xvf ~/apache-ant-1.10.3-bin.tar.gz
mv ~/apache-ant-1.10.3 ~/ant
echo "PATH=\"/root/ant/bin:\$PATH\"" >> ~/.bashrc
source ~/.bashrc

cd ~
wget http://apache.rediris.es/zookeeper/zookeeper-3.4.11/zookeeper-3.4.11.tar.gz
tar -xvf zookeeper-3.4.11.tar.gz 
cd zookeeper-3.4.11
ant clean tar
tar -C/opt -xvf build/zookeeper-3.4.11.tar.gz 
...
cd ~
wget http://apache.rediris.es/hadoop/common/hadoop-2.9.0/hadoop-2.9.0-src.tar.gz
tar -xvf ~/hadoop-2.9.0-src.tar.gz
mv ~/hadoop-2.9.0-src ~/hadoop-src
cd ~/hadoop-src
mvn package -Pdist,native -DskipTests -Dtar -Dzookeeper.version=3.4.11
tar -C/opt -xvf ~/hadoop-src/hadoop-dist/target/hadoop-2.9.0.tar.gz
...
cd ~
wget http://apache.rediris.es/spark/spark-2.3.0/spark-2.3.0.tgz
tar -xvf spark-2.3.0.tgz 
cd ~/spark-2.3.0
dev/make-distribution.sh --name custom-spark --tgz "-Pyarn,hadoop-2.7" -Dhadoop.version=2.9.0 -DskipTests
tar -C/opt -xvf spark-2.3.0-bin-custom-spark.tgz 
cd /opt
mv spark-* spark
...
cd ~
wget http://apache.rediris.es/hbase/1.4.3/hbase-1.4.3-src.tar.gz
tar -xvf hbase-1.4.3-src.tar.gz 
cd hbase-1.4.3
mvn clean package assembly:single -DskipTests -Dhadoop.version=2.9.0 -Dzookeeper.version=3.4.11
tar -C/opt -xvf hbase-assembly/target/hbase-1.4.3-bin.tar.gz
...
cd ~
wget http://apache.rediris.es/hive/hive-2.3.3/apache-hive-2.3.3-src.tar.gz
tar -xvf apache-hive-2.3.3-src.tar.gz
cd apache-hive-2.3.3-src
...
mvn clean package -Dhadoop.version=2.9.0 -DskipTests -Pdist
tar -C/opt -xvf ./packaging/target/apache-hive-2.3.3-bin.tar.gz
...
cd ~
wget http://apache.rediris.es/pig/pig-0.17.0/pig-0.17.0-src.tar.gz
tar -xvf pig-0.17.0-src.tar.gz 
cd pig-0.17.0-src
sed -i '/<\/dependencies>/i \
    <dependency org="org.apache.hadoop" name="hadoop-hdfs-client" rev="${hadoop-hdfs.version}" conf="hadoop2->master"><artifact name="hadoop-hdfs-client" ext="jar" /></dependency>' ivy.xml
sed -i 's/target name="package" depends="jar, docs/target name="package" depends="jar/g' build.xml 
ant -Dhadoopversion=2 -Dzookeeper.version=3.4.11 -Dhadoop-common.version=2.9.0 -Dhadoop-hdfs.version=2.9.0 -Dhadoop-mapreduce.version=2.9.0 tar
...
cd ~
wget http://apache.rediris.es/flume/1.8.0/apache-flume-1.8.0-src.tar.gz
tar -xvf apache-flume-1.8.0-src.tar.gz 
cd apache-flume-1.8.0-src
mvn package -DskipTests -DsourceJavaVersion=1.8 -DtargetJavaVersion=1.8 -Dhadoop2.version=2.9.0 -Dhive.version=2.3.3
tar -C/opt -xvf  flume-ng-dist/target/apache-flume-1.8.0-bin.tar.gz 
...
cd sqoop-1.4.7
ant tar -Dhadoop.version=2.9.0 -Dhcatalog.version=2.3.3
tar -C/opt -xvf build/sqoop-1.4.7.bin__hadoop-2.9.0.tar.gz
mv /opt/sqoop-* /opt/sqoop
cp ~/mysql-connector-java-5.1.46/mysql-connector-java-5.1.46-bin.jar /opt/sqoop/lib/
